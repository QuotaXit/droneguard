...
export async function removeUserPersonalStorage({
   ...
   * anche se i percorsi arrivassero alterati,
   * eliminiamo soltanto file appartenenti
   * realmente all'utente.
   */
  const safeAvatarPaths =
    avatarPaths.filter((path) => {
      const normalizedPath =
        String(path || "")
          .trim()
          .replace(/^\/+/, "")
      ...
      return isOwnedAvatarPath(
        normalizedPath,
        normalizedUserId
      )
    })

  const safeCertificationPaths =
    certificationPaths.filter((path) => {
      const normalizedPath =
        String(path || "")
          .trim()
          .replace(/^\/+/, "")
